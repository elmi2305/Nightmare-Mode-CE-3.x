...
public class RenderObsidianCreeper extends RenderCreeper {
    ...
    protected ResourceLocation getCreeperTextures(EntityCreeper par1EntityCreeper) {
        if (par1EntityCreeper instanceof EntityObsidianCreeper) {
            return OBSIDIAN_CREEPER_TEXTURE;
        } else {return super.getEntityTexture(par1EntityCreeper);}
    }
    @Override
    protected ResourceLocation getEntityTexture(Entity par1Entity) {
        if(par1Entity instanceof EntityObsidianCreeper) {
            return OBSIDIAN_CREEPER_TEXTURE;
        } else {
    ...
    protected int renderCreeperPassModel(EntityObsidianCreeper par1EntityCreeper, int par2, float par3) {
    ...
    protected int shouldRenderPass(EntityLivingBase par1EntityLivingBase, int par2, float par3) {
        return this.renderCreeperPassModel((EntityObsidianCreeper)par1EntityLivingBase, par2, par3);
    }
}
