...
public final class StorageColor {
    ...
    private static int chestRenderColorOverride = -1;
    ...
    public static boolean dyeMinecart(EntityMinecart minecart, EntityPlayer player, ItemStack dye) {
        if (!(minecart instanceof IColoredChest chest)) {
            return false;
        }
        int color = mixChestColor(chest.nm$hasChestColor() ? chest.nm$getChestColor() : DEFAULT_CHEST_COLOR,
                dye.getItemDamage());
        chest.nm$setChestColor(color);
        consumeHeldItem(player, dye);
        return true;
    ...
    public static boolean washMinecart(EntityMinecart minecart, EntityPlayer player, ItemStack soap) {
        if (!(minecart instanceof IColoredChest chest)) {
            return false;
        }
        chest.nm$setChestColor(WHITE);
    ...
    public static void setChestRenderColorOverride(int color) {
        chestRenderColorOverride = color & 0xFFFFFF;
    }

    public static void clearChestRenderColorOverride() {
        chestRenderColorOverride = -1;
    }

    public static boolean hasChestRenderColorOverride() {
        return chestRenderColorOverride >= 0;
    }

    public static int getChestRenderColorOverride() {
        return chestRenderColorOverride;
    }
